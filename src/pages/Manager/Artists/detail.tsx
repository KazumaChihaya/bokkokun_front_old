...
import { ArtistDetail, BoxInvoice, EarnInvoice } from '@/services/manager/artist';
import { antize } from '@/libs/icon';
import { MdOutlineDomain } from 'react-icons/md';
import { Link } from 'react-router-dom';
import { FormOutlined, TagsOutlined } from '@ant-design/icons';
import { Artist, ActiveBox, ReserveBox, useArtist } from '@/services/manager/artist';
...
import { Pool, usePool, usePoolCalc } from '@/services/manager/pool';
...
const activeBoxColumns: ColumnsType<ActiveBox> = [
  {
    ...
    render: (_, { box, id }) => (
      <div style={{ minWidth: '8em' }}>
        <Link to={`/manager/boxes/${box?.id}`}>{(box ? box?.box_category?.code : '')+box?.id}</Link>
      </div>
    ),
